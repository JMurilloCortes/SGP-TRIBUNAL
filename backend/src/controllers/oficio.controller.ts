...
export async function generarOficio(req: AuthRequest, res: Response) {
  ...
  switch (data.modeloId) {
    ...
    case 4:
      ...
      break
    default:
      return res.status(400).json({ message: 'Modelo no implementado' })
  }
  ...
  await prisma.actuacion.create({
    data: {
      ...
      descripcion: `Se generó oficio: ${modelo.nombre} dirigido a ${data.destinatario} - ${data.entidad}`,
    },
  })

  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
  res.setHeader('Content-Disposition', `attachment; filename="oficio_${proceso.radicado}.docx"`)
  res.send(buffer)
}
